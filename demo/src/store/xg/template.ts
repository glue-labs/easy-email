...
import { template } from '@demo/services/xg/template';

export function getAdaptor(data): IEmailTemplate {
  const content = JSON.parse(data).content as IBlockData;
  return {
    content,
    subject: data.title,
...
export default createSliceState({
  ...
  effects: {
    fetchById: async (
      ...
      try {
        let data = await template.getTemplateById(id);

        return getAdaptor(data.templateJson);
      } catch (error) {
        history.replace('/');
        throw error;
      }
